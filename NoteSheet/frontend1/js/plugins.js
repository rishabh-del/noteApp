...
! function (a, b, c, d) {
  ...
  var i = "vide",
    ...
    k = /iPad|iPhone|iPod/i.test(d.userAgent),
    l = /Android/i.test(d.userAgent);
  h.prototype.init = function () {
    ...
    }), d.$video.bind("loadedmetadata." + i, function () {
      d.$video.css("visibility", "visible"), d.resize()
    }), d.$element.bind("resize." + i, function () {
      d.resize()
    }))
    ...
    this.$element.unbind(i), this.$video && this.$video.unbind(i), delete a[i].lookup[this.index], this.$element.removeData(i), this.$wrapper.remove()
  }, a[i] = {
    lookup: []
  }, a.fn[i] = function (b, c) {
    var d;
    ...
    a(b).bind("resize." + i, function () {
      for (var b, c = a[i].lookup.length, d = 0; c > d; d++) b = a[i].lookup[d], b && b.resize()
